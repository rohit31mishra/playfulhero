({
    ...
    uploadHelp :function(component, event){
        var fileInput = component.find("fileId").get("v.files");
        // get the first file using array index[0]  
        var file = fileInput[0];
        ...
        objFileReader.onload = $A.getCallback(function() {
            var fileContents = objFileReader.result;
            var base64 = 'base64,';
            console.log('fileContents  r  '+fileContents);
            var filec = encodeURIComponent(fileContents);
            var dataStart = fileContents.indexOf(base64) + base64.length;
            fileContents = fileContents.substring(dataStart);
            // call the uploadProcess method 
            console.log('fileContents '+fileContents);
            //self.uploadProcess(component, file, fileContents);
            var action = component.get("c.saveChunk1");
            action.setParams({
                'file' : fileContents,
            });
            action.setCallback(this, function(response) {
                console.log('hello '+response.getReturnValue());
                let mapVal = response.getReturnValue();
